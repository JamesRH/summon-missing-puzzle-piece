...
class ImageAligner:
    ...
    def process(self, complete_image_path, missing_image_path, output_path, save_intermediate=False, show_matches=False, draw_contours=False, save_svg_contour=False, print_width_inches=None, print_height_inches=None):
        ...
        if complete_image is None or missing_image is None:
            raise FileNotFoundError(f"Could not load one or both input images. Check your file paths: {complete_image_path}, {missing_image_path}")
        ...
        # Store the uncropped final_output for TIFF before cropping for PNG and SVG dimensions
        final_output_uncropped = final_output.copy()

        # Find bounding box of the aligned white mask for cropping
        coords = cv2.findNonZero(aligned_white_mask)
        if coords is not None:
            x, y, w, h = cv2.boundingRect(coords)
            # Crop the final_output image to this bounding box
            final_output = final_output[y:y+h, x:x+w]
        else:
            if self.debug:
                print("Warning: No white region found for cropping. Output PNG will be empty.")
        ...
        if save_svg_contour:
            ...
            for contour in contours:
                ...
                # Adjust contour points to be relative to the cropped bounding box origin
                points = (approx_contour - (x, y)).reshape(-1, 2).tolist()
...
def main():
    ...
    parser.add_argument('complete_image_path', type=str, nargs='?', help='Path to the complete image (Image 1). Required unless --batchdir is used.')
    parser.add_argument('missing_image_path', type=str, nargs='?', help='Path to the missing image (Image 2). Required unless --batchdir is used.')
    parser.add_argument('output_path', type=str, nargs='?', help='Path to save the final output image (e.g., replacement.png). Required unless --batchdir is used.')
    parser.add_argument('--batchdir', type=str, help='Optional: Directory containing subdirectories to process in batch mode.')
    parser.add_argument('--complete_filename', type=str, default='complete.jpg', help='Default filename for the complete image within each batch subdirectory. Used with --batchdir.')
    parser.add_argument('--missing_filename', type=str, default='missing.jpg', help='Default filename for the missing image within each batch subdirectory. Used with --batchdir.')
    parser.add_argument('--output_filename', type=str, default='replacement.png', help='Default filename for the output image within each batch subdirectory. Used with --batchdir.')
    ...
    if args.batchdir:
        if not os.path.exists(args.batchdir):
            print(f"Error: Batch directory '{args.batchdir}' does not exist.")
            return
        if not os.path.isdir(args.batchdir):
            print(f"Error: Batch directory '{args.batchdir}' is not a directory.")
            return

        print(f"Processing images in batch mode from '{args.batchdir}'...")
        processed_count = 0
        skipped_count = 0

        for subdir_name in os.listdir(args.batchdir):
            current_subdir_path = os.path.join(args.batchdir, subdir_name)

            if os.path.isdir(current_subdir_path):
                print(f"\n--- Processing subdirectory: '{subdir_name}' ---")
                batch_complete_image_path = os.path.join(current_subdir_path, args.complete_filename)
                batch_missing_image_path = os.path.join(current_subdir_path, args.missing_filename)
                batch_output_path = os.path.join(current_subdir_path, args.output_filename)

                if not os.path.exists(batch_complete_image_path):
                    print(f"Warning: Complete image '{args.complete_filename}' not found in '{subdir_name}'. Skipping.")
                    skipped_count += 1
                    continue
                if not os.path.exists(batch_missing_image_path):
                    print(f"Warning: Missing image '{args.missing_filename}' not found in '{subdir_name}'. Skipping.")
                    skipped_count += 1
                    continue

                try:
                    if aligner.debug:
                        print(f"  Complete: {batch_complete_image_path}")
                        print(f"  Missing: {batch_missing_image_path}")
                        print(f"  Output: {batch_output_path}")

                    aligner.process(
                        batch_complete_image_path,
                        batch_missing_image_path,
                        batch_output_path,
        ...
        print(f"\nBatch processing complete. Processed {processed_count} subdirectories, skipped {skipped_count}.")
        return

    # Single image processing logic (existing logic)
    try:
        # Check if files exist before running
        if not (args.complete_image_path and args.missing_image_path and args.output_path):
            raise ValueError("complete_image_path, missing_image_path, and output_path are required in single image mode.")

        # Mutually exclusive arguments check and aspect ratio validation
        # This check is now performed using the uncropped image's actual pixel dimensions.
        # This ensures consistency with the image content's inherent aspect ratio.
